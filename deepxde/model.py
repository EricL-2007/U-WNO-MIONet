...
class Model:
    ...
    def _test(self):
        ...
        n_test = self.train_state.X_test[0].shape[0]
        # Chunk over the branch (sample) axis in groups of `batch_size` to bound
        # memory use, same as training; this used to be hardcoded to 4 samples
        # (== n_test) which silently broke for any other test-set size.
        batch_size = min(4, n_test)
        timestep_batch_size = self.timestep_batch_size
        n_branch_chunks = int(np.ceil(n_test / batch_size))
        n_time_chunks = int(self.training_time_size/timestep_batch_size)
        for i in range(n_branch_chunks):
            start, end = i*batch_size, min((i+1)*batch_size, n_test)
            cur_batch_size = end - start
            for j in range(n_time_chunks):
                y_pred_test_timestepbatch, loss_test_batch = self._outputs_losses(
                    False,
                    (self.train_state.X_test[0][start:end],
                    self.train_state.X_test[1][start:end],
                    self.train_state.X_test[2].reshape(self.training_time_size,-1)[j*timestep_batch_size:(j+1)*timestep_batch_size,:].reshape(timestep_batch_size,-1)),
                    self.train_state.y_test.reshape(n_test,self.training_time_size,96,200)[start:end,:][:,j*timestep_batch_size:(j+1)*timestep_batch_size,:].reshape(cur_batch_size,-1),
                    self.train_state.test_aux_vars,
                    range(start,end),
                )
                y_pred_test_timestepbatch = y_pred_test_timestepbatch.reshape(cur_batch_size,timestep_batch_size,96,200)
                if j == 0:
            ...
            if i == 0:
                ...
                self.train_state.y_pred_test = np.concatenate((self.train_state.y_pred_test,y_pred_test_batch),0)
        self.train_state.loss_test /= n_branch_chunks*n_time_chunks
        self.train_state.y_pred_test = self.train_state.y_pred_test.reshape(n_test,-1)
